...
strOutputPath       = 'E:\Dropbox (SquareCircleMITtoo)\_y_Code\zzResultsFolder\eachfiledatatrain\';
...
% filenamebasearray{7} = 'DrawData_Tower48_2021-02-02_to2021-02-09';  
% filenamebasearray{8} = 'DrawData_Tower48_2021-02-23_to2021-03-02';  
% filenamebasearray{9} = 'DrawData_Tower48_2021-03-02_to2021-03-09';  
% filenamebasearray{10} = 'DrawData_Tower48_2021-03-09_to2021-03-16';  
% filenamebasearray{11} = 'DrawData_Tower48_2021-03-16_to2021-03-23';  
...
%         nnin_miniBatchSize          = 200;%200;
        ...
        tempstr = ['rev7_' filenamebase '__trEACH' '_maxE' num2str(nnin_maxEpochs) '_mx' num2str(subbatchMaxLen) '_drop' num2str(nnin_LearnRateDropPeriod) '_lstm350lstm0' '_filter' num2str(PrefltLEN)];
        %tempstr = ['rev7_' filenamebase '__trEACH' '_maxE' num2str(nnin_maxEpochs) '_mx' num2str(subbatchMaxLen) '_drop' num2str(nnin_LearnRateDropPeriod) '_lstm100lstm50' '_filter' num2str(PrefltLEN)];
...
%                             regressionLayer];
                        ...
                        %delete(poolobj)
                        %save('fourthOrderMdlnet','trainedNetworkModel','XTrainTRANSPOSE','YTrainTRANSPOSE');
                    ...
                    % 
                    %     %stl_train_arxarmax
                    %     %stl_train_NLarxarmax_allt
                    %     if(bRNNnarx)
                    %         [fitnet, Ypredict, hiddenSizes]              = stl_train_RNNnarx_allt_function(Y,x);
                    %         [fitnet_filt, Ypredict_filt, hiddenSizes_filt]    = stl_train_RNNnarx_allt_function(Y_filt,x);
                    %     else
                    %         %stl_train_arxarmax
                    %         %stl_train_NLarxarmax_allt
                    %         [fitsys1, Ypredict, fitsys1_filt, Ypredict_filt]  = stl_train_NLarxarmax_allt_function(dsiddata,dsiddata_filt, x, nImportantColumns);
                    %         %[fitsys1, Ypredict, fitsys1_filt, Ypredict_filt]  = stl_trainedload_function(nWhichBatch,x);
                    %     end
                    % 
                        ...
                        modelYOutResponse = predict(trainedNetworkModel,x');
                    ...
                    %     bPlot = 1;
                    %     if(bPlot)
                    %         % Plot resutls of fit time series
                    %         strlistlegend = [];
                    %         figure(1000+nWhichBatch)
                    %         plot(Y,'k');                        strlistlegend{length(strlistlegend)+1} = 'Y';
                    %         hold on
                    %     %    plot(sample_indexfilt,Y_filt,'g');  strlistlegend{length(strlistlegend)+1} = 'Y_filt';
                    %         plot(Ypredict,'r');                 strlistlegend{length(strlistlegend)+1} = 'Ypredict';
                    %     %    plot(Ypredict_filt,'b');            strlistlegend{length(strlistlegend)+1} = 'Ypredict_filt';
                    %         %axis([1 length(Y) min(Y) max(Y)]);
                    %         %axis([1 1300 -.4 .6]);
                    %     %    axis([1 length(Y) loBFD-meanY hiBFD-meanY]);
                    %         axis([1 length(Y) loBFD hiBFD]);
                    %         if(bMeanRemove)
                    %              axis([1 length(Y) loBFD-meanY hiBFD-meanY]);
                    %         end
                    %         
                    ...
                    %         title(['Training: Batch ' num2str(nWhichBatch) '.  Sub ' num2str(nWhichSub) '. ']);
                    %     end
                    % 
                    end
                %     if(bRNNnarx)
                %         fitnet_ARRAY{nWhichBatch} = fitnet;
                %         fitnet_filt_ARRAY{nWhichBatch} = fitnet_filt;
                %     else
                %         fitsys1_ARRAY{nWhichBatch} = fitsys1;
                %         fitsys1_filt_ARRAY{nWhichBatch} = fitsys1_filt;
                %     end

                    trainedNetworkModel_ARRAY{nWhichBatch} = trainedNetworkModel;
                    XTrainTRANSPOSE_ARRAY{nWhichBatch} = XTrainTRANSPOSE;
                    YTrainTRANSPOSE_ARRAY{nWhichBatch} = YTrainTRANSPOSE;

%                     tempstr = ['tmp51batch' num2str(nWhichBatch) '.mat'];
%                     save(tempstr);
                    
                    filenameRslt= [tempstr '_' num2str(nWhichBatch) '.mat']; 
